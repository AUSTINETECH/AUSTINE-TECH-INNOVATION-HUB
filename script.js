...
function initField() {
    ...
    const soil = new THREE.Mesh(
        new THREE.PlaneGeometry(18, 10, 34, 18),
    ...
    for (let i = -5; i <= 5; i++) {
        const c = new THREE.CatmullRomCurve3([
            new THREE.Vector3(i * .72 + 2.4, -1.54, -4.6),
            new THREE.Vector3(i * .62 + 2.1, -1.2, -1.8),
            new THREE.Vector3(i * .48 + 1.8, -.84, 1.4),
            new THREE.Vector3(i * .32 + 1.4, -.42, 4.6)
        ]);
    ...
    grp.add(new THREE.Line(
        new THREE.BufferGeometry().setFromPoints(npts.map(p => new THREE.Vector3(p[0], p[1], p[2]))),
        new THREE.LineBasicMaterial({ color: 0xffffff, transparent: true, opacity: .15 })
    ));
    ...
    function anim(t) {
        ...
        nmesh.forEach(m => {
            if (m.userData.o !== undefined) {
                m.scale.setScalar(1 + Math.sin(s * 1.8 + m.userData.o) * .26);
            }
        });
...
const PROJS = [
    {
        ...
        lang: 'HTML',
        status: 'live',
        desc: 'HTML project shipped live to Vercel. First verified live deployment from the studio.',
        img: 'https://images.unsplash.com/photo-1487058792275-0ad4aaf24ca7?auto=format&fit=crop&w=600&q=80',
        repo: 'https://github.com/Greenbird122/git-vizor',
        live: 'https://git-vizor.vercel.app',
        col: 0x0052FF
    },
    {
        id: 'my-portifolio',
        ...
        repo: 'https://github.com/Greenbird122/my-portifolio',
        live: 'https://my-portifolio-six-pearl.vercel.app',
        col: 0x00E5FF
    },
    {
        id: 'urban-umbrella',
        ...
        repo: 'https://github.com/Greenbird122/urban-umbrella',
        live: null,
        col: 0xFF6B00
    },
    {
        id: 'probable-goggles',
        name: 'probable-goggles',
        lang: 'TypeScript',
        status: 'repo',
        ...
        repo: 'https://github.com/Greenbird122/probable-goggles',
        live: null,
        col: 0xFF6B00
    },
    {
        id: 'symmetrical-couscous',
        name: 'symmetrical-couscous',
        lang: 'TypeScript',
        status: 'repo',
        ...
        repo: 'https://github.com/Greenbird122/symmetrical-couscous',
        live: null,
        col: 0xFF6B00
    },
    {
        id: 'ideal-funicular',
        name: 'ideal-funicular',
        lang: 'TypeScript',
        status: 'offline',
        ...
        repo: 'https://github.com/Greenbird122/ideal-funicular',
        live: null,
        col: 0x4A8AFF
    },
    {
        ...
        repo: 'https://github.com/Greenbird122',
        live: null,
        col: 0x00E5FF
    }
];
...
function filt(f, btn) {
    ...
    if (scO.nodeGroups) {
        scO.nodeGroups.forEach((ng, i) => {
            const p = PROJS[i];
            ng.visible = f === 'all' ||
                (f === 'live' && p.status === 'live') ||
                (f === 'ts' && p.lang.includes('TypeScript')) ||
                (f === 'html' && p.lang.includes('HTML'));
        });
    }
}
...
function renderList() {
    ...
    if (!el) return;
    const vis = PROJS.filter(p =>
        activeF === 'all' ||
        (activeF === 'live' && p.status === 'live') ||
        (activeF === 'ts' && p.lang.includes('TypeScript')) ||
        (activeF === 'html' && p.lang.includes('HTML'))
    );
...
function selProj(id) {
    ...
    if (scO.nodeGroups) {
        ...
        scO.nodeGroups.forEach((ng, i) => {
            if (ng.children[0]) {
                ng.children[0].scale.setScalar(i === idx ? 1.5 : 1);
            }
        });
    }
}
...
function initScene() {
    ...
    const orbRing = new THREE.Mesh(
        new THREE.TorusGeometry(14, .04, 8, 80),
        new THREE.MeshBasicMaterial({ color: 0x10172B, transparent: true, opacity: .5 })
    );
    ...
    PROJS.forEach((p, i) => {
        ...
        const mat = new THREE.MeshPhongMaterial({
            color: p.col,
            emissive: new THREE.Color(p.col).multiplyScalar(.12),
            shininess: 80,
            transparent: true,
            opacity: .95
        });
        ...
        if (p.status === 'live') {
            const rm = new THREE.Mesh(
                new THREE.RingGeometry(sz + .15, sz + .28, 36),
                new THREE.MeshBasicMaterial({
                    color: 0x00E5FF,
                    transparent: true,
                    opacity: .55,
        ...
        const pm = new THREE.Mesh(
            new THREE.SphereGeometry(sz + .25, 18, 18),
            new THREE.MeshBasicMaterial({
                color: p.col,
                transparent: true,
                opacity: .12,
    ...
    cv.addEventListener('wheel', e => {
        radius = Math.max(7, Math.min(42, radius + e.deltaY * .04));
        camPos();
        e.preventDefault();
    }, { passive: false });
    ...
    cv.addEventListener('touchmove', e => {
        if (e.touches.length === 1 && lt) {
            theta -= (e.touches[0].clientX - lt.x) * .01;
    ...
    cv.addEventListener('click', e => {
        ...
        if (hits.length && hits[0].object.userData.pid) {
            selProj(hits[0].object.userData.pid);
        }
    });
    ...
    function anim(t) {
        ...
        nodeGroups.forEach((ng, i) => {
            ...
            ng.children.forEach(c => {
                if (c.userData.pu) {
                    const sc = 1 + Math.sin(s * 1.5 + c.userData.off) * .25;
